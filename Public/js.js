...
// {
//     window.history.replaceState("","",window.location.href="http://localhost:9000/todo")
// }
start();
function start() {
    ...
    request.addEventListener("load",function(){
        ...
        // console.log(data)
       let filtered= data.filter((todo)=>{ return todo.added_by===username})
        console.log("filtered",filtered)
        // filtered.sort(function(x, y){ return y.created_at - x.created_at })
        // filtered.sort(function(x, y){ return new Date(y.created_at) - new Date(x.created_at) })
   ...
   function handler(event){
    ...
    if(event.code=="Enter" && inputfield.value!="")
     {
          data = { title: inputfield.value, status: "Pending", id: Date.now() ,added_by:username }
   ...
   container.style.borderBottom='solid 1px #ded9d9'
